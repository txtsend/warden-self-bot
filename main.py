...
lencommands=0
import os
clear=lambda: os.system(f'cls && title Selfbot by WARDEN {version} ' if os.name == 'nt' else 'clear')
try:
	import discord
...
if theme=='standart':
	color={'Intro': Fore.BLUE, 'Info_name': Fore.MAGENTA, 'Info_value': Fore.YELLOW}
elif theme=='discord':
...
Intro=color['Intro']+"""
   _____ ______ _      ______ ____   ____ _______  __          __     _____  _____  ______ _   _ 
  / ____|  ____| |    |  ____|  _ \ / __ \__   __| \ \        / /\   |  __ \|  __ \|  ____| \ | |
 | (___ | |__  | |    | |__  | |_) | |  | | | |     \ \  /\  / /  \  | |__) | |  | | |__  |  \| |
  \___ \|  __| | |    |  __| |  _ <| |  | | | |      \ \/  \/ / /\ \ |  _  /| |  | |  __| | . ` |
  ____) | |____| |____| |    | |_) | |__| | | |       \  /\  / ____ \| | \ \| |__| | |____| |\  |
 |_____/|______|______|_|    |____/ \____/  |_|        \/  \/_/    \_\_|  \_\_____/|______|_| \_|
...
if on_command_error:
	...
	async def on_command_error(ctx, error):
		...
		try: await ctx.send(f'**__Selfbot by WARDEN__\n\nПроизошла ошибка :x:\n```{error}```**')
		except: pass
@bot.event
async def on_command(ctx):
	time=datetime.now().strftime('%H:%M:%S')
	arguments=ctx.message.content.replace(pref+ctx.invoked_with, '')
	print(f'{Fore.BLUE_EX}[{time}] {Fore.GREEN_EX}{pref}{ctx.invoked_with}{Fore.LIGHTGREEN_EX}{arguments}{Fore.RESET}')
@bot.event
